...
print("Populating 6 new global/US wildfire monitoring regions into PostgreSQL grid_cells (FAST)...")
...
for reg_id, cfg in regions.items():
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            cells_data.append((current_id, reg_id, poly_wkt, centroid_lon, centroid_lat, 10000))
            cell_idx += 1
    ...
    template = "(%s, %s, extensions.st_setsrid(extensions.st_geomfromtext(%s), 4326), extensions.st_setsrid(extensions.st_point(%s, %s), 4326), %s)"
    
    execute_values(cur, query, cells_data, template=template, page_size=1000)

cur.execute("SELECT * FROM public.get_available_regions();")
rows = cur.fetchall()

print("\n--- COMPLETE REGIONS LIST FROM DATABASE RPC ---")
for r in rows:
